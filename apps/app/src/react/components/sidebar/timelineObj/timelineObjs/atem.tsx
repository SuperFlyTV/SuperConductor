...
export const EditTimelineObjAtemAny: React.FC<{ objs: TimelineObjAtemAny[]; onSave: OnSave }> = observer(
	function EditTimelineObjAtemAny({ objs, onSave: onSave0 }) {
		...
		if (allTimelineObjsHaveSameDeviceId) {
			...
			if (md?.metadataType === MetadataType.ATEM) {
				deviceMetadata = md
			}
		}
							...
							<>
								...
								<div className="setting">
									<IntInput
										...
										{...inputValue(
											objs,
											(obj) => (obj.content.ssrcProps as any).borderOuterWidth,
											0
										)}
										onChange={(v) => {
											if (!firstObj.content.ssrcProps.borderEnabled) return
								...
								<div className="setting">
									<IntInput
										...
										{...inputValue(
											objs,
											(obj) => (obj.content.ssrcProps as any).borderInnerWidth,
											0
										)}
										onChange={(v) => {
											if (!firstObj.content.ssrcProps.borderEnabled) return
								...
								<div className="setting">
									<IntInput
										...
										{...inputValue(
											objs,
											(obj) => (obj.content.ssrcProps as any).borderSaturation,
											0
										)}
										onChange={(v) => {
											if (!firstObj.content.ssrcProps.borderEnabled) return
