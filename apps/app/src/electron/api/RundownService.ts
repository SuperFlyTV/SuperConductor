...
export class RundownService extends EventEmitter {
	constructor(private app: Application<ServiceTypes, any>, private rundownRepository: EverythingService) {
		super()
	}

	async get(id: string, params: Params): Promise<Rundown> {
		// TODO: access control
		const rundown = this.rundownRepository.getRundown({ rundownId: id })
	...
	async create(data: { name: string }): Promise<Rundown> {
		// TODO: access control
		const result = await this.rundownRepository.newRundown(data)
	...
	async playPart(data: { rundownId: string; groupId: string; partId: string }): Promise<void> {
		// TODO: access control
		const rundown = await this.rundownRepository.playPart(data)
	...
	async pausePart(data: { rundownId: string; groupId: string; partId: string }): Promise<void> {
		// TODO: access control
		const rundown = await this.rundownRepository.pausePart(data)
		this.emit('updated', rundown) // TODO: make this more granular
	}
	...
	async stopPart(data: { rundownId: string; groupId: string; partId: string }): Promise<void> {
		// TODO: access control
		const rundown = await this.rundownRepository.stopPart(data)
		this.emit('updated', rundown) // TODO: make this more granular
	}
}
